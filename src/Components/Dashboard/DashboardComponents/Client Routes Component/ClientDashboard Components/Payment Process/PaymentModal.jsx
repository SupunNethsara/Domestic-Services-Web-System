...
import { FaCreditCard, FaCashRegister, FaCopy, FaCheck, FaCheckCircle } from 'react-icons/fa';
...
const PaymentModal = ({ worker, onClose, stripePromise }) => {
    ...
    const fetchBankDetails = async () => {
      try {
        const response = await axios.get(
          `/api/workers-bank-details/${worker?.id}`
        );
  ...
  const handlePaymentSuccess = (message = 'Payment completed successfully!') => {
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`flex items-center justify-center p-3 rounded-md border transition-colors ${paymentMethod === 'card'
              ...
              <button
                ...
                className={`flex items-center justify-center p-3 rounded-md border transition-colors ${paymentMethod === 'bank'
                    ? 'border-indigo-500 bg-indigo-50 text-indigo-700'
                    : 'border-gray-300 hover:bg-gray-50'
                  } ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''}`}
                disabled={isProcessing}
          ...
          {paymentMethod === 'card' ? (
            <Elements stripe={stripePromise}>
              <StripePaymentForm
                ...
                onSuccess={() => handlePaymentSuccess(`Your payment of LKR ${paymentAmount} was successful!`)}
                onCancel={onClose}
              />
            </Elements>
          ) : (
            <div className="mt-4">
              ...
              <div className="flex justify-end mt-6">
                <button
                  type="button"
                  onClick={() => {
                    handlePaymentSuccess(`Bank transfer instructions for LKR ${paymentAmount} have been copied!`);
                    copyToClipboard(`Transfer LKR ${paymentAmount} to ${bankDetails.account_holder_name}, ${bankDetails.bank_name}, Account: ${bankDetails.account_number}`);
                  }}
                  className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors"
                  disabled={isProcessing || !bankDetails}
                >
                  Confirm Transfer
                </button>
              </div>
